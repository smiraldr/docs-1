...
**Example — IO Intelligence (io.net) through a local proxy.** IO Intelligence serves an OpenAI-compatible Chat Completions API rather than the Anthropic format, so run [claude-code-router](https://github.com/musistudio/claude-code-router) as a local translation proxy:

<Steps>
  <Step title="Install the router">
    Requires Node.js 22–24 (the router's SQLite dependency does not yet support Node 25).
    ...
    `ccr ui` opens the router's management UI at `http://127.0.0.1:3458`.
  </Step>
  <Step title="Add the IO Intelligence provider">
    Under **Providers**, add a provider with a custom endpoint: Base URL `https://api.intelligence.io.solutions/api/v1`, your IO Intelligence API key, protocol **OpenAI Chat**, and the models to expose (for example, `meta-llama/Llama-3.3-70B-Instruct`).
  </Step>
  <Step title="Start the gateway and create a client key">
    Start the router — the gateway listens on `http://127.0.0.1:3456`. Then copy a client key from the router's **API Keys** page (for example, the default Local Gateway key); Verdent uses this key to authenticate to the gateway.
  </Step>
  <Step title="Add the endpoint in Verdent">
    In Verdent, add an **Anthropic-Compatible (Custom Endpoint)** provider with Base URL `http://127.0.0.1:3456`, the router client key as the API key, and the models configured in the router — enter model ids as configured (for example, `meta-llama/Llama-3.3-70B-Instruct`). Then **Save** and **Test Connection**.
  </Step>
</Steps>
