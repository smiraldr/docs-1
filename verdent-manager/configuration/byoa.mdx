...
<Steps>
  ...
  <Step title="Add the endpoint in Verdent">
    In Verdent, add an **Anthropic-Compatible (Custom Endpoint)** provider with Base URL `http://127.0.0.1:3456`, the router client key as the API key, and the models configured in the router — enter model ids as configured (for example, `meta-llama/Llama-3.3-70B-Instruct`). Then click **Test Connection** and **Save**.
  </Step>
</Steps>
