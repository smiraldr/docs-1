...
1. Install and open the router (Node.js 22+):
...
2. Under **Providers**, add a provider with a custom endpoint: Base URL `https://api.intelligence.io.solutions/api/v1`, your IO Intelligence API key, protocol **OpenAI Chat**, and the models to expose (for example, `meta-llama/Llama-3.3-70B-Instruct`).
3. Start the router — the gateway listens on `http://127.0.0.1:3456`.
4. In Verdent, add an **Anthropic-Compatible (Custom Endpoint)** provider with Base URL `http://127.0.0.1:3456` and the models configured in the router, then **Save** and **Test Connection**.
